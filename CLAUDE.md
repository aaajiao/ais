...
- **可视化 hook 必须 `staleTime: 0` + `refetchOnMount: 'always'`（v1.5 起）**: `useVisualizationData()` 服务 `/visualize` 的 4 个 view 共享一次数据拉取。契约是 **"进入页面即看到当前快照"** —— 用户在 Artworks / Editions / Locations 改完回来必须立刻看见改动。**不要**复制 `useDashboard.ts` 模式（其默认 5min cache + `refetchOnWindowFocus: true`，面向频繁停留的首页；viz 面向偶尔访问的快照）。`refetchOnWindowFocus` 关闭是为了避免 SVG 在切窗口时跳动重渲染。如果新增 viz 用 hook，**保持同一组配置**，别引入 cache 节流。守护测试 `src/pages/__tests__/Visualize.test.tsx`。详见 [docs/visualize.md](docs/visualize.md)。
