...
- **外部 API 自动继承 L2 归一化（v1.3.4 起）**: `/api/external/v1/query` 直接调 `tool.execute(params)`（绕过 zod schema 校验），但因为 L2 normalize-filters 是写在每个工具 execute() 入口里的，外部客户端发来的 default-padded payload **自动**被同样物理拦截——即便外部 LLM 走 OpenAI structured outputs 强填 `''/0/null`，read-only 路径也安全。**关键约束**：未来重构 search 工具时，**绝不能**把归一化逻辑从 execute() 入口提到外面（例如挪到 chat.ts 的 onCall 钩子），否则外部 API 路径会失去保护。`api/external/v1/schema.ts` 的 SCHEMA 常量需要标 `nullable: true` + 维护 `parameter_handling` 章节告诉外部客户端"传 null 别传 ''/0"，与内部 OpenAI prompt 的 null-instruction 是同一份契约的两份外化（一份给我们的 LLM，一份给外部 LLM）。守护测试 `api/external/__tests__/schema.test.ts` 断言 nullable + parameter_handling 不掉。文档见 `docs/external-api.md` "参数处理铁律"。
