...
- **Anthropic-only providerOptions 必须按 provider 分支**: `cacheControl`（prompt caching）等 Anthropic-only 字段只能注入到 Anthropic 路径，按 `getProviderName(model)` 判断；千万不要给 OpenAI 路径带上，会导致请求体异常。`contextManagement.compact_20260112` 还有额外的模型子集约束，单独由 `supportsCompactBeta(modelId)` 门控（见下条）。
- **`compact_20260112` 是 Anthropic 子集模型特性，绝不能按 provider 一刀切（v1.5.x 修复）**: `contextManagement.compact_20260112` 只支持 **Sonnet 4.6+ / Opus 4.6+/4.7 / mythos-preview**。发给 Haiku 4.5 / Sonnet 4.5 / 老 Claude 模型会被服务端拒，错误以 "Conversation too long" 形式上抛 —— **即使对话历史为空也失败**，迷惑性极高。注入条件**必须**用 `supportsCompactBeta(modelId)`（`api/lib/model-provider.ts`），不能写 `provider === 'anthropic'`。注入失败时走 `prepareMessagesForModel({ useServerSideCompaction: false })` 客户端截断兜底，跟 OpenAI 同款。Anthropic 出新模型时查 https://platform.claude.com/docs/en/build-with-claude/compaction 文档确认后再扩 `COMPACT_SUPPORTED_MODELS` 白名单。守护测试 `api/lib/__tests__/model-provider.test.ts` + `api/lib/__tests__/message-utils.test.ts`。
...
- **public schema 新建表必须显式 GRANT 给 Data API 角色（migration 007 起，2026-10-30 起 Supabase 默认）**: Supabase 2026-04-28 公告 public schema 表不再自动暴露到 Data API —— 2026-05-30 起新建项目默认 opt-out，2026-10-30 起所有现有项目（含本项目）同步翻转。本项目已通过 migration 007 提前 opt-in：未来任何 `CREATE TABLE public.X` 必须紧跟 `GRANT SELECT, INSERT, UPDATE, DELETE ON X TO authenticated, service_role`（按需 `GRANT SELECT ... TO anon`），否则前端 `supabase-js` / 外部 API Key / 任何 PostgREST 调用直接返回 `42501 permission denied`。**grant 是 RLS 之外的另一层** —— grant 控制角色能否触及表，RLS 控制能看到哪些行；缺 grant 时 RLS 根本不会被评估。范式见 `supabase/schema.sql` 的 `DATA API EXPOSURE` 段和 `supabase/migrations/archived/007_data_api_explicit_grants.sql`。**绝不**靠 "Supabase 历史默认" 这种隐式行为；schema.sql 必须自给自足（fresh deploy 到新项目能直接跑通）。详见 [docs/database.md `v1.x Data API Exposure 默认翻转`](docs/database.md#v1x-data-api-exposure-默认翻转)。
...
- **备份 ZIP 走 Vercel Blob Private Store + Function 中转 + v2.3.3 SDK 字面陷阱（migration 009 起）**: 工作室数据备份系统的存储后端是 Vercel Blob 不是 Supabase Storage —— Supabase Free 单文件 50MB 装不下 ~340MB 完整快照；Vercel Pro Blob 单文件 5TB 完全够。**私有性走 Dashboard 创建 Store 时的 Access mode 设置**（必选 Private，创建后不可改），**不是**调用时参数。`@vercel/blob` v2.3.3 SDK 实际 API 与 docs 有差异：(1) `put()` 的 `access` 参数字面**只接受 `'public'`**（写 `'private'` typecheck 直接报错），私有性靠 store 级别决定；(2) **SDK 不导出 `get()`**（docs 写了未发布），下载用 `list({ prefix }) + fetch(url, { Authorization: 'Bearer ${BLOB_READ_WRITE_TOKEN}' })` 替代。下载必须走 `/api/export/backup/download` 经 `verifyAuth` + stream 中转，**绝不**给前端返回 Blob URL —— private store URL "不可猜但永久泄漏"，是 security-by-obscurity 反模式。`BLOB_READ_WRITE_TOKEN` 由 Vercel 自动从连接的 Blob Store 注入；SDK 自动读取，不要手动 export。验证 SDK 实际 API 看 `node_modules/@vercel/blob/dist/index.d.ts` 而非 docs。所有路径 / 文件名约定集中在 `api/lib/backup/storage.ts` 单一来源。详见 [docs/backup.md](docs/backup.md)。
- **备份恢复是覆盖语义 + Rollback Gate 是 UX 闸门服务端是最终权威（migration 009 起）**: `/api/import/backup` 不是合并/upsert，是**先 DELETE 当前用户全部用户域表行 → INSERT 备份内容**（保留原 UUID 让 `gallery_links` 公开 URL 在恢复后继续有效）。合并对"字段被改坏想还原"场景无效（ID 相同会双向 upsert），只有覆盖能给真 point-in-time 还原。**安全闸门**：前端 UI 强制用户先点 `下载当前回滚包` 调 `/api/export/rollback` 把当前状态 ZIP 落到本地 → 输入 `CONFIRM` 双确认 → 发请求时带 `X-Rollback-Confirmed: true` header。**前端 UI 只是 UX**，服务端 handler 检查这个 header 不通过返 `412 Precondition Failed` 是真正的拦截。还有三层服务端校验：manifest.user_id ≠ 当前用户 → 403 cross_account；schema_version 不匹配 → 400 schema_mismatch（不做自动迁移）；format_version 不匹配 → 400 format_mismatch。**`users` 表全程不碰**（账号身份保护，备份只是工作室数据）。守护测试 `api/lib/backup/__tests__/restore.test.ts`（12 tests 覆盖 DELETE 反 FK 顺序 / INSERT 正 FK 顺序 / 图片 fallback / 批量插入）。
