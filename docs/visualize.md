...
- **三档视觉词汇（v1.6.x 第二轮，信息密度递减）**：Diaspora 把每个 outflow / archive 元素按"有多少身份信息"分三档，brutalist 单色逻辑下用三种几何形态承载：
  ...
  | anonymous dust | outflow + 无买家信息 | 灰实心几何小圆 r=3.5 opacity=0.55 | hover 看 sale_date，**不可点击** |
  | **ghost editions ring** | non-outflow + 无 location（"待补全档案 inbox"） | **空心**几何小圆 r=4 opacity=0.55 在 R=245 外圈 | **click → /editions/:id 去补 location** |

  外形语义：organic blob = 具象有形（有身份）/ 灰实心 = 看得见但无名 / 空心 = 空缺待填。三档不混着用 —— 不让 anonymous 看上去像 ghost，也不让 ghost 看上去像 entity。

  **R 分布**（centered 800×600 viewBox）：
  - R 60–190 → time-spiral（location / named / anonymous dated 全混）
  - R 220 → undated 推到这里
  - R 245 → ghost editions ring（可点击补全 inbox）
...
- **Constellation 形态：time-spiral（v1.6 起，替换 v1.6 早期同心环 + type-arc 模型；v1.6.x 二次调整）**：Diaspora 视图早期版本按 `location.type` 把 inner-ring 切成 5 段弧度（museum / private_collection / gallery / studio / other），实测发现 14 个 location 中 12 个都是 museum（artist outflow 集中流向美术馆），结果 60° museum 弧度严重过载、其它 200° 弧度全空。新模型抛弃 type 分弧，改用 **time-spiral：径向距离 = 第一次交易时间，节点大小 = type + editionCount**：
  ...
  - **径向（r）= 第一次交易时间**（A 方向：老→近 center，呼应 Strata 地层学）：
    ...
    - **角度由 phyllotaxis 黄金角分配 + deterministic 碰撞推开**（v1.6.x 第二轮）：先按 `ms(sale_date)` 升序排，第 i 个 dated point `angle = -π/2 + i × GOLDEN_ANGLE`（≈137.5° / 段，与 2π 不可通约 → 任意 N 下相邻 index 散得最开，向日葵种子密堆原理）。**r 仍由真实时间决定**，但 angle 跟时间脱钩。phyllotaxis 已大幅减少重叠，但 R_INNER 附近圆周短、r 又接近时仍可能 chord 不足；最多 8 轮迭代遍历每对 (i, j)（j 时间更晚），笛卡尔距离 < r_i + r_j + 4 就把 j 的 angle 推 0.06 rad（i 始终锚定时间）—— 固定迭代顺序 + 固定步长 → 同输入同输出。**旧 (v1.6.x 一轮) sorted-index uniform angle `(i/N)·2π` 在 N=37 时实测留 10 对节点重叠，最严重 chord 比 r_a+r_b 还少 14px**，phyllotaxis + repel 修复。
    - undated entity（缺 `sale_date`，全部 outflow editions 都没有）推到 `R_GHOST = 220` 外圈均匀分布。**"缺失数据不藏"原则的几何化**（呼应 ghost editions ring / Strata DegenerateGlyph）
  ...
  - **节点形状：organic blob（v1.6.x 起，v1.6.x 第二轮扰动加强）**：location + named_private 节点不再用规则 `<circle>`，而是 `generateOrganicPath(cx, cy, baseR, seed=node.id)` 生成的 **8 段** quadratic-bezier 闭合 path —— 按 entity id 做字符 hash → 径向扰动 **±25%** → 每个 entity 一份独一无二的有机轮廓，render 间稳定（deterministic）。**v1.6.x 第二轮：12 段 ±15% → 8 段 ±25%** —— 旧扰动在 named_private (r≈7-9) 上只 1-2px，肉眼仍是规则圆；新扰动达 1.75-3.5px，blob 形态清晰。视觉用意：跟 Strata 严格 geometric `<rect>` 方块 + Markets 抽象 `<circle>` dot 形成对照 —— Diaspora 节点表达"具象的个体性"。**anonymous dust 仍是 `<circle r=3.5>`**（灰实心，不应用 organic blob —— "看得见但无名"档专用形态）。selection ring 仍是 `<circle r=baseR×1.3>` 几何环 —— "几何 ring 抓住有机 blob"的 brutalist 单色 contrast。**约束**：不引入随机库 / 动画 / morph，~30 行纯函数；扰动幅度 / 段数变了会触发整轮 snapshot diff，慎改。
  ...
  - **anonymous outflow（v1.6.x 时间螺旋 + 第二轮视觉升级）**：每条 anonymous outflow edition 一个独立 `<circle r=3.5>` dust dot，跟 location / named_private 共享同一份 time-spiral 映射 —— dated anonymous 按 sale_date 落在 [R_INNER, R_OUTER_DATA] 区间，undated 推 R_GHOST 外圈。旧实现把 anonymous 聚合到外圈 `ANONYMOUS_R = 310` 均匀分布；v1.6.x 一轮改"每件作品独立落点"；**v1.6.x 第二轮把 r 从 1.5 升到 3.5、opacity 从 0.3 升到 0.55**，让 anonymous 变成"灰实心几何小圆 = 看得见但无名"档（vs entity 的有机壳和 ghost 的空心圆）。每个 dust 父 `<g>` 带 `<title>` 暴露 sale_date 给浏览器原生 tooltip。**仍不可点击 / 不进 hover 状态机 / 无 navigate 目标**：anonymous 没有 entity 身份，每个 dot 只代表一次匿名流出。**selection 命中时 opacity=1（不画 ring）** —— ring 跨 dot 视觉混乱（selection by artwork 可能匹配多个 anonymous edition）。
  - **Ghost editions ring（v1.6.x 第二轮新增，待补全档案 inbox）**：`buildGhostEditions(editions, artworks)` 筛 `location_id == null && status ∉ (sold, gifted)` 的 edition；`layoutGhostRing(ghosts, { width, height })` 在 R=245 外圈均匀分布。视觉是**空心**几何小圆 `<circle r=4 fill="none" stroke-foreground opacity=0.55>`，每个圆包一层 `<g role="button" tabIndex={0}>`，**click / Enter / Space 跳 `/editions/:editionId`** 让用户去补 location。`<title>` 显示 `{title} · {inventoryNumber} · {status}`，aria-label 走 i18n `diaspora.constellation.aria.ghost`。**排序**：status 优先级（`in_production` → `in_studio` → `in_transit` → `at_gallery` → `at_museum` → 其他），组内按 `created_at` desc（晚创建在前 —— 最新被忽视的更优先补全）。无 ghost 时不渲染 untracked hint stat 行。设计哲学：信息密度递减的三档视觉规范的最外层，呼应原则 9「缺失态画出来不藏」—— 把"等待补充元数据的项"做成可操作的 inbox，而不是装饰圆。注意：与匿名 dust 的关键差异 —— anonymous = 已离开 + 无买家（不可点击），ghost = 未离开 + 无 location（可点击）。**不**做 artwork-level ghost（无 edition 的 artwork），那不在 Diaspora 数据模型范围。
...
| `diasporaUtils.test.ts` | 91 — 节点关联 / 中心选择 tie-breaker / 同心环布局 / 边聚合 / tracked stat / `getGhostNodes`（M2 @deprecated）/ `buildConstellation`（M6 + firstSaleDate）/ `layoutConstellation` time-spiral / **phyllotaxis 黄金角 angle 分配（v1.6.x 第二轮）+ 碰撞推开（密集 fixture 所有 chord ≥ r_a+r_b-1）** / `getNodeVisual`（含 v1.6.x 第二轮 **anonymous r=3.5 + opacity=0.55**）/ `generateOrganicPath`（**v1.6.x 第二轮 8 段 + ±25% 扰动**）/ **`buildGhostEditions`（过滤 outflow + 无 location / title 回退链 / status 优先级排序 + created_at desc 组内 / 空输入）** / **`layoutGhostRing`（N=0 空数组 / R=245 默认 / -π/2 起点 / 均匀分布 / 自定义 radius / GhostRingPoint 带 ghost meta）** / `namedNodeRadius`（legacy @deprecated） |
| `DiasporaView.test.tsx` | 46 — 初始状态 / hover 预览 / hover 离开 / click pin / edition 行跳转 / view all 跳转 / 二次 click 取消 pin / 切换 pin / pin 时 hover 不干扰 / 空数据 / aria-pressed / 键盘 Enter / 长名 SVG `<title>` / pin 卡片 stopPropagation / spy stopPropagation / **per-edition ghost circle 渲染 + 空心 stroke-only + click navigate + Enter/Space 键盘 navigate（v1.6.x 第二轮 ghost editions inbox）/ legend 含 anonymous + untracked chip（v1.6.x 第二轮）/ stat 区 untrackedHint 文案（v1.6.x 第二轮）/ 无 ghost 时不渲染 hint（v1.6.x 第二轮）** / Constellation artist / location / named_private / anonymous nodes（M6）/ Phase 2 selection ring × 3（M3a）/ private_collection inner stroke ring（v1.6）/ label radial anchor phyllotaxis 角度切换（v1.6.x 第二轮）/ dated+undated location 都渲染（v1.6）/ **organic blob 节点 / anonymous dots r=3.5（v1.6.x 第二轮升级）/ anonymous `<title>` 暴露 sale_date** |
